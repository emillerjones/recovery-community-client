...
const categoryId = (category) => `discount2-${category.id}`;
...
export default function DiscountLinks2() {
  ...
  useEffect(() => {
    ...
    fetch(`${API}/api/discount2`, { signal: controller.signal })
      .then(async (response) => {
        ...
        if (!response.ok) throw new Error(data.message || "Discount2 could not be loaded.");
        return data;
      })
  ...
  if (!content) {
    return <main className="discount-page discount2-page"><div className="discount2-state"><h1>{error ? "Discount2 is temporarily unavailable." : "Loading Discount2…"}</h1>{error && <><p>{error}</p><Link to="/discountlinks">View the current discount page</Link></>}</div></main>;
  }

  return (
    <main className="discount-page discount2-page">
            ...
            return (
              <section className="discount-category" id={categoryId(category)} key={category.id}>
                ...
                <div className="discount-grid">
                  ...
                  {!visibleBrands.length && <p className="discount2-empty">No active discounts in this category right now.</p>}
                </div>
              </section>
            );
          })}
